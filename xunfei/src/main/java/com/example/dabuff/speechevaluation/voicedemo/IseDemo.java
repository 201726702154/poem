    ...
    private EvaluatorListener mEvaluatorListener = new EvaluatorListener() {
        ...
        public void onResult(EvaluatorResult result, boolean isLast) {
            ...
            if (isLast) {
                ...
                if (!TextUtils.isEmpty(builder)) {
                    mResultEditText.setText("测试结束，请查看结果");
                }
        ...
        public void onError(SpeechError error) {
            ...
            if (error != null) {
                ...
                mResultEditText.setHint("请点击“开始测试”按钮");
            } else {
...
//                break;
        if (view.getId() == R.id.ise_start) {
            ...
            if (evaText.length() >= 20) {
                evaText = evaText.substring(0, 20) + "...";
            }
//            evaText = evaText;?
            //.replaceAll("。", "").replaceAll("，", "").replaceAll(" ", "").replaceAll(" ", "");
            //求之不得寤寐思服悠哉悠哉辗转反侧参差荇菜左右采之窈窕淑女琴瑟友之参差荇菜左右芼之窈窕淑女钟鼓乐之
        ...
        if (view.getId() == R.id.ise_stop) {
            if (mIse.isEvaluating()) {
                mResultEditText.setHint("测试已停止，等待结果中...");
                mIse.stopEvaluating();
            }
        }
        if (view.getId() == R.id.ise_cancel) {
            mIse.cancel();
            mIseStartButton.setEnabled(true);
            mResultEditText.setText("");
            mResultEditText.setHint("请点击“开始测试”按钮");
    ...
    private void setEvaText() {
        ...
        if ("en_us".equals(language)) {
            ...
            if ("read_syllable".equals(category)) {
                ...
                Intent intent = getIntent();
                String strs = intent.getStringExtra("data");
